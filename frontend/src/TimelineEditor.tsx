...
export function TimelineEditor({engine,tracks}:{engine:BrowserEngine;tracks:Track[]}){
  ...
  return <div className="session-editor">
    <div className="timeline-tools"><span>SESSION TIME · drag white playhead to scrub/re-hear · click ruler to seek · drag title to move</span><label>ZOOM <input aria-label="Timeline zoom" type="range" min="3" max="24" step="1" value={zoom} onChange={e=>setZoom(+e.target.value)}/></label><button onClick={()=>{if(scroll.current)scroll.current.scrollLeft=Math.max(0,120+engine.sessionPosition*zoom-scroll.current.clientWidth/2);}}>Find playhead</button></div>
    ...
    <div className="timeline-hint" role="status">{scrubbing!==null?`Scrubbing to ${label(scrubbing)} — release to jump and re-hear audio`:drag?`${drag.kind==='move'?'Start':'Source cue'} ${label(drag.value)} — release to apply`:message||'Drag the white playhead or click the ruler to scrub & re-hear transitions. Purple end bands show crossfades.'}</div>
  </div>;
}
function ClipWave({peaks,cue,length,duration}:{peaks:number[];cue:number;length:number;duration:number}){
  const ref=useRef<HTMLCanvasElement>(null);
  useEffect(()=>{const canvas=ref.current;if(!canvas)return;const draw=()=>{
  ...
  };draw();const observer=new ResizeObserver(draw);observer.observe(canvas);return()=>observer.disconnect();},[peaks,cue,length,duration]);
