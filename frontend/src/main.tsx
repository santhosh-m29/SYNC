...
type Track = { id:string; title:string; filename:string; duration:number; bpm:number; key:string|null };
type EngineState = { current_track:string; position:number; playing:boolean; auto_dj:boolean; volume:number; queue_order:string[]; upcoming:string[]; transition_state:string; event?:{next_track:string;outgoing_transition_timestamp:number;incoming_start_timestamp:number;transition_duration:number;strategy:string}|null; errors:string[] };
const endpoint = import.meta.env.VITE_ENGINE_URL || 'http://127.0.0.1:8765';
...
function App(){
 ...
 const current=byId(state?.current_track), cue=byId(selected); return <main>
  ...
  <section className="cue"><div className="section-title"><div><label>CUE EDITOR</label><h2>{cue?.title||'Select a track'}</h2></div><button disabled={!selected} onClick={()=>selected&&send('next',{track:selected})}>Load track</button></div><div className="wave">{wave.length?wave.map((v,i)=><i key={i} style={{height:`${Math.max(3,v*100)}%`}}/>):<span>Audio waveform will load from the local engine</span>}</div><p>Click/select a track above. Cue editing stays inside the local engine boundary.</p></section>
  <footer>{state?.errors?.at(-1)||notice}<span>Frontend deployment is independent of Python audio runtime</span></footer>
 </main>;
}
