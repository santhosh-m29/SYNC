...
function App() {
    ...
    return <main>
        ...
        <div className="import-status" role="status">{uploadNotice && <span>{uploadNotice}</span>}{tracks.filter(t => t.status === 'error').map(t => <div key={t.id}>{t.filename} — {t.error}</div>)}{connected && !tracks.length}</div>
